/*
 * Copied from https://github.com/alphagov/govuk_frontend_toolkit/blob/master/javascripts/govuk/details.polyfill.js
 */
  ...
  GOVUK.details = {
    ...
    addDetailsPolyfill: function (list, container) {
      ...
      for (i; i < n; i++) {
        ...
        details.__link = details.getElementsByTagName('a').item(0);
        ...
        if (openAttr === true) {
            ...
            if (details.__link) details.__link.removeAttribute('tabindex');
        } else {
            details.__summary.setAttribute('aria-expanded', 'false');
            details.__content.setAttribute('aria-hidden', 'true');
            if (details.__link) details.__link.setAttribute('tabindex', '-1');
          if (!GOVUK.details.NATIVE_DETAILS) {
    ...
    statechange: function (summary) {
      ...
      summary.__details.__content.setAttribute('aria-hidden', (hidden ? 'false' : 'true'));
        ...
        if (summary.__details.__link) {
            if (hidden === true) {
                summary.__details.__link.removeAttribute('tabindex')
            } else {
                summary.__details.__link.setAttribute('tabindex', '-1')
            }
        }
