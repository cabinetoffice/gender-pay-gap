...
namespace GenderPayGap.WebUI.Tests.Controllers
{
    ...
    public partial class SubmitControllerTests
    {
        ...
        public async Task SubmitController_CheckData_GET_Success()
        {
            ...
            var testService = new SubmissionService(null, null, testDraftFileBL, null);
        ...
        [Test]
        [Ignore("To be discussed in code review : This situation cannot happen now as we use Draft")]
        [Description("Ensure Check Data throws session expired when no stashed Return View Model")]
        public async Task SubmitController_CheckData_GET_When_Return_ViewModel_Not_Stashed_SessionExpired()
        {
            // Arrange
            ...
            // ASSERT
            Assert.NotNull(result);
            Assert.AreEqual(result.ViewName, "CustomError");

            var model = result.Model as ErrorViewModel;
            Assert.NotNull(model);
            Assert.That(model.ErrorCode == 1134);
        }


        [Test]
        [Description("Ensure that CheckData form has all previous form values correct and validated and sent successfully")]
        public async Task SubmitController_CheckData_POST_Success()
        {
            // Arrange
            var testUserId = 1000;
            ...
            Assert.NotNull(model, "Expected unstashed model");
        ...
        public async Task SubmitController_CheckData_POST_When_ModelState_Is_Invalid_Then_Returns_Error()
        {
            ...
            var user = new User {UserId = 1, EmailAddress = "magnuski@hotmail.com", EmailVerifiedDate = VirtualDateTime.Now};
            var address = new OrganisationAddress {
                OrganisationId = 1, Address1 = "Address line 1", PostCode = "PC1", Status = AddressStatuses.Active
            };
            var organisation = new Organisation {OrganisationId = 1, SectorType = SectorTypes.Private};
            var userOrganisation = new UserOrganisation {
                OrganisationId = organisation.OrganisationId,
        ...
        public async Task SubmitController_EmployerWebsite_POST_When_ViewModel_Not_Valid_Then_Return_Error()
        {
            ...
            //mock entered return CompanyLinkToGPGInfo in the CompanyLinkToGPGInfo EmployerWebsite view
            var returnViewModel = new ReturnViewModel {
                CompanyLinkToGPGInfo = "httpx://www.google.com", FirstName = string.Empty, ReportInfo = new ReportInfoModel()
            };
        ...
        [Description(
            "If a user does not have a return existing in the database, " + "a new one should be created and verified with default values")]
        public async Task SubmitController_EnterCalculations_GET_UserHasNoReturn_ShowNewPrivateSectorReturn_Delete_me()
        {
            // Arrange:
            Organisation testOrganisation = OrganisationHelper.GetPublicOrganisation();
            User testUser = UserHelper.GetNotAdminUserWithVerifiedEmailAddress();
            UserOrganisation testUserOrganisation = UserOrganisationHelper.LinkUserWithOrganisation(testUser, testOrganisation);
            ...
            mockedDatabaseToConfig
                .Setup(x => x.GetAll<User>())
                .Returns(new[] {testUser}.AsQueryable);

            mockedDatabaseToConfig
                .Setup(x => x.GetAll<Organisation>())
                .Returns(new[] {testOrganisation}.AsQueryable);

            mockedDatabaseToConfig
                .Setup(x => x.GetAll<UserOrganisation>())
            ...
            // System.InvalidCastException : Unable to cast object of type 'System.Web.Mvc.RedirectToRouteResult' to type 'System.Web.Mvc.ViewResult'.
            controller.Bind(result.Model);
            var resultModel = result.Model as ReturnViewModel;

            //Assert:
            Assert.That(resultModel.ReturnId == 0, "ReturnId expected 0");
            //Assert.That(resultModel.OrganisationId == 1698, "Organisation Id ");

            //Assert.That(new ReturnViewModel() {}, )};
        ...
        [Description(
            "If a user does not have a return existing in the database, a new one should be created and verified with default values")]
        public async Task SubmitController_EnterCalculations_GET_UserHasNoReturn_ShowNewPublicSectorReturn()
        {
            ...
            var user = new User {UserId = 1, EmailAddress = "magnuski@hotmail.com", EmailVerifiedDate = VirtualDateTime.Now};
            var organisation = new Organisation {OrganisationId = 1};
            var userOrganisation = new UserOrganisation {OrganisationId = 1, UserId = 1, PINConfirmedDate = VirtualDateTime.Now, PIN = "0"};
        ...
        public async Task SubmitController_EnterCalculations_GET_UserHasReturn_ShowExistingReturn_delete_me()
        {
            ///TODO: Create mocks for the needed data (user, organisation, userOrganisatoin, return)
            // Arrange:
            var user = new User {UserId = 1, EmailAddress = "magnuski@hotmail.com", EmailVerifiedDate = VirtualDateTime.Now};
            var organisation = new Organisation {OrganisationId = 1};
            var userOrganisation = new UserOrganisation {
        ...
        [Test(Author = "Oscar Lagatta")]
        [Description(
            "If a user in the private sector does not have a return existing in the database, a new one should be created and verified with default values")]
        public async Task
            SubmitController_EnterCalculations_GET_When_User_Has_No_Return_Then_A_New_Private_Sector_Return_Should_Be_Created()
        {
            User mockedUser = UserHelper.GetNotAdminUserWithVerifiedEmailAddress();
            ...
            Draft actualDraftFile = await testDraftFileFileBusinessLogic.GetExistingOrNewAsync(
                ...
                mockedUser.UserId);
            actualDraftFile = await testDraftFileFileBusinessLogic.UpdateAsync(returnViewModel, actualDraftFile, mockedUser.UserId);
            ...
            actualDraftFile = await testDraftFileFileBusinessLogic.GetExistingOrNewAsync(
                actualReturnViewModel.OrganisationId,
                actualReturnViewModel.AccountingDate.Year,
                mockedUser.UserId);
            Assert.NotNull(actualDraftFile);
        ...
        public async Task SubmitController_EnterCalculations_POST_EmptyFields_ShowAllErrors()
        {
            //ARRANGE:
            ///TODO: create mock of the following (user, organisation and userOrganisation) 
            var user = new User {UserId = 1, EmailAddress = "magnuski@hotmail.com", EmailVerifiedDate = VirtualDateTime.Now};
            ...
            //ACT:
            var result = await controller.EnterCalculations(model) as ViewResult;
            ...
            Assert.Multiple(
                () => {
                    ...
                    Assert.AreEqual(
                        result.ViewData.ModelState.IsValidField("DiffMeanBonusPercent"),
                        false,
                        "Expected DiffMeanBonusPercent failure");
                    Assert.AreEqual(
                        result.ViewData.ModelState.IsValidField("DiffMeanHourlyPayPercent"),
                        false,
                        "Expected DiffMeanHourlyPayPercent failure");
                    Assert.AreEqual(
                        result.ViewData.ModelState.IsValidField("DiffMedianBonusPercent"),
                        false,
                        "Expected DiffMedianBonusPercent failure");
                    Assert.AreEqual(
                        result.ViewData.ModelState.IsValidField("DiffMedianHourlyPercent"),
                        false,
                        "Expected DiffMedianHourlyPercent failure");

                    Assert.AreEqual(
                        result.ViewData.ModelState.IsValidField("FemaleLowerPayBand"),
                        false,
                        "Expected FemaleLowerPayBand failure");
                    Assert.AreEqual(
                        result.ViewData.ModelState.IsValidField("FemaleMedianBonusPayPercent"),
                        false,
                        "Expected FemaleMedianBonusPayPercent failure");
                    Assert.AreEqual(
                        result.ViewData.ModelState.IsValidField("FemaleMiddlePayBand"),
                        false,
                        "Expected FemaleMiddlePayBand  failure");
                    Assert.AreEqual(
                        result.ViewData.ModelState.IsValidField("FemaleUpperPayBand"),
                        false,
                        "Expected FemaleUpperPayBand  failure");
                    Assert.AreEqual(
                        result.ViewData.ModelState.IsValidField("FemaleUpperQuartilePayBand"),
                        false,
                        "Expected FemaleUpperQuartilePayBand  failure");

                    Assert.AreEqual(
                        result.ViewData.ModelState.IsValidField("MaleLowerPayBand"),
                        false,
                        "Expected MaleLowerPayBand  failure");
                    Assert.AreEqual(
                        result.ViewData.ModelState.IsValidField("MaleMedianBonusPayPercent"),
                        false,
                        "Expected MaleMedianBonusPayPercent  failure");
                    Assert.AreEqual(
                        result.ViewData.ModelState.IsValidField("MaleMiddlePayBand"),
                        false,
                        "Expected MaleMiddlePayBand  failure");
                    Assert.AreEqual(
                        result.ViewData.ModelState.IsValidField("MaleUpperPayBand"),
                        false,
                        "Expected MaleUpperPayBand  failure");
                    Assert.AreEqual(
                        result.ViewData.ModelState.IsValidField("MaleUpperQuartilePayBand"),
                        false,
        ...
        public async Task SubmitController_EnterCalculations_POST_FemaleBonusSetCannotExceed100percent_ShowsErrors()
        {
            ...
            var testFileRepository = UiTestHelper.DIContainer.Resolve<IFileRepository>();
            var submissionServiceMock = new SubmissionService(null, null, new DraftFileBusinessLogic(testFileRepository), null);
        ...
        [Description("SubmitController EnterCalculations POST test fields are now using regex to ensure only 1 decimal place")]
        public void SubmitController_EnterCalculations_POST_Fields_Now_Use_Regex_To_Ensure_One_Decimal_Place()
        {
            ///TODO: create a real mock of the following (user, organisation and userOrganisation)
            /// 
            // this skeleton method was included as there was this to do note on a file
        ...
        public async Task SubmitController_EnterCalculations_POST_MaxInValidValues_ShowAllErrors()
        {
            ///TODO: create mock of the following (user, organisation and userOrganisation)
            // Arrange
            var user = new User {UserId = 1, EmailAddress = "magnuski@hotmail.com", EmailVerifiedDate = VirtualDateTime.Now};
        ...
        public async Task SubmitController_EnterCalculations_POST_MinInValidValues_ShowAllErrors()
        {
            ///TODO: create mock of the following (user, organisation and userOrganisation) 
            // Arrange
            var user = new User {UserId = 1, EmailAddress = "magnuski@hotmail.com", EmailVerifiedDate = VirtualDateTime.Now};
        ...
        [Test(Author = "Oscar Lagatta")]
        [Ignore("To Be Refactor as Per Save Draft Logic Change")]
        [Description("Enter calculations should fail when values are empty")]
        public async Task SubmitController_EnterCalculations_POST_When_Return_Has_No_Values_Then_Show_All_Errors()
        {
            User mockedUser = UserHelper.GetNotAdminUserWithVerifiedEmailAddress();
            Organisation mockedOrganisation = OrganisationHelper.GetPrivateOrganisation();
            UserOrganisation mockedUserOrganisation =
            ...
            mockedDatabase
                .Setup(x => x.GetAll<User>())
                .Returns(new[] {mockedUser}.AsQueryable);

            mockedDatabase
                .Setup(u => u.GetAll<Organisation>())
                .Returns(new[] {mockedOrganisation}.AsQueryable);

            mockedDatabase
                .Setup(uo => uo.GetAll<UserOrganisation>())
                .Returns(new[] {mockedUserOrganisation}.AsQueryable);
            ...
            ModelStateEntry emptyModelState = controller.ModelState[""];
            ModelError emptyModelReportedError = emptyModelState.Errors.First();
            Assert.AreEqual("There’s a problem with your data entry", emptyModelReportedError.ErrorMessage);

            ModelStateEntry femaleUpperQuartilePayBandModelState = controller.ModelState["FemaleUpperQuartilePayBand"];
            ModelError femaleUpperQuartilePayBandModelReportedError = femaleUpperQuartilePayBandModelState.Errors.First();
            Assert.AreEqual("There’s a problem with your data entry", emptyModelReportedError.ErrorMessage);

            ModelStateEntry femaleUpperPayBandModelState = controller.ModelState["FemaleUpperPayBand"];
            ModelError femaleUpperPayBandModelStateModelReportedError = femaleUpperPayBandModelState.Errors.First();
            Assert.AreEqual(
                "Please try again - figures for each quartile must add up to 100%",
                femaleUpperPayBandModelStateModelReportedError.ErrorMessage);

            ModelStateEntry femaleLowerPayBandModelState = controller.ModelState["FemaleLowerPayBand"];
            ModelError femaleLowerPayBanddModelStateModelReportedError = femaleLowerPayBandModelState.Errors.First();
            Assert.AreEqual(
                "Please try again - figures for each quartile must add up to 100%",
                femaleLowerPayBanddModelStateModelReportedError.ErrorMessage);
        }
        ...
        public async Task SubmitController_PersonResponsible_GET_Success()
        {
            // Arrange
            var user = new User {
                UserId = new Random().Next(1000, 9999), EmailAddress = "magnuski@hotmail.com", EmailVerifiedDate = VirtualDateTime.Now
            };
            var organisation = new Organisation {OrganisationId = 1, SectorType = SectorTypes.Private};
            organisation.OrganisationId = new Random().Next(1000, 9999);
            var userOrganisation = new UserOrganisation {
        ...
        public async Task SubmitController_PersonResponsible_POST_EmptyFields_ShowAllErrors()
        {
            // Arrange
            var user = new User {UserId = 1, EmailAddress = "magnuski@hotmail.com", EmailVerifiedDate = VirtualDateTime.Now};
            var organisation = new Organisation {OrganisationId = 1};
            ...
            var model = new ReturnViewModel {JobTitle = emptyString, FirstName = emptyString, LastName = emptyString};
            ...
            Assert.Multiple(
                () => {
                    ...
                    Assert.AreEqual(
                        result.ViewData.ModelState.IsValidField("JobTitle"),
                        false,
                        "Expected JobTitle value other than empty strings failure");
                    Assert.AreEqual(
                        result.ViewData.ModelState.IsValidField("FirstName"),
                        false,
                        "Expected FirstName value other than empty strings  failure");
                    Assert.AreEqual(
                        result.ViewData.ModelState.IsValidField("LasttName"),
                        false,
                        "Expected LasttName value other than empty strings  failure");
        ...
        public async Task SubmitController_PersonResponsible_POST_NullFields_ShowAllErrors()
        {
            // Arrange
            var user = new User {UserId = 1, EmailAddress = "magnuski@hotmail.com", EmailVerifiedDate = VirtualDateTime.Now};
            var organisation = new Organisation {OrganisationId = 1};
            ...
            Assert.Multiple(
                () => {
                    ...
                    Assert.AreEqual(
                        result.ViewData.ModelState.IsValidField("JobTitle"),
                        false,
                        "Expected JobTitle  value other than null failure");
                    Assert.AreEqual(
                        result.ViewData.ModelState.IsValidField("FirstName"),
                        false,
                        "Expected FirstName value other than null  failure");
                    Assert.AreEqual(
                        result.ViewData.ModelState.IsValidField("LasttName"),
                        false,
                        "Expected LasttName value other than null  failure");
                });
            //TODO you should also be checking modelstate has no other errors to ensure other fields are being retained after postback and not causing errors
        }
        ...
        public async Task SubmitController_PersonResponsible_POST_ValidFields_NoErrors()
        {
            // Arrange
            var user = new User {UserId = 1, EmailAddress = "magnuski@hotmail.com", EmailVerifiedDate = VirtualDateTime.Now};
            var organisation = new Organisation {OrganisationId = 1};
            ...
            Assert.Multiple(
                () => {
                    ...
                    Assert.AreEqual(
                        result.ViewData.ModelState.IsValidField("JobTitle"),
                        true,
                        "Expected JobTitle value other than empty strings success");
                    Assert.AreEqual(
                        result.ViewData.ModelState.IsValidField("FirstName"),
                        true,
                        "Expected FirstName value other than empty strings  success");
                    Assert.AreEqual(
                        result.ViewData.ModelState.IsValidField("LasttName"),
                        true,
                        "Expected LastName value other than empty strings  success");
                });
            //TODO you should also be checking modelstate has no other errors to ensure other fields are being retained after postback and not causing errors
        }
        ...
        [Test]
        [Description("Ensure that Person Responsible form is filled and sent successfully")]
        [Ignore("To Be Refactor Based On Save Draft")]
        public async Task SubmitController_PersonResponsible_POST_When_Model_State_Is_Not_Valid()
        {
            // Arrange
            var user = new User {UserId = 1, EmailAddress = "magnuski@hotmail.com", EmailVerifiedDate = VirtualDateTime.Now};
            var organisation = new Organisation {OrganisationId = 1, SectorType = SectorTypes.Private};
            ...
            //3.Check that the result is not null
            Assert.NotNull(result);
            Assert.AreEqual(controller.ViewData.ModelState.IsValidField("FirstName"), false);
            Assert.AreEqual(controller.ViewData.ModelState.IsValidField("LastName"), false);
            Assert.AreEqual(controller.ViewData.ModelState.IsValidField("JobTitle"), false);
        }
        ...
        public void SubmitController_SubmissionComplete_POST_Success()
        {
            ...
            //controller.Request = 
            // var bb = Mock.Of<System.Web.HttpRequestBase>();
            // System.Web.HttpContextBaseExtensions
            // bb.GetOwinContext();
            // Act
            var result = controller.SubmissionCompletePost(string.Empty) as ViewResult;
        }
        // identify the tests
        // create all the arrange first. 
        // what existing helpers we can use and what we need to create


        // These 'to do' messages were found on the original file ('original file' was a file that contained a subset of the submit controller tests)
        //TODO For non Positive Tests:
        //TODO shouldn't you be checking that all the model fields are returned correctly
        //TODO remember when checking modelstate.isvalid=true there should be no need to check individual modelstate errors but when checking  checking modelstate.isvalid=valid you should be checking all the errors are exactly right with no more and no less errors than expected
