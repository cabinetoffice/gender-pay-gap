...
namespace GenderPayGap.WebUI.Tests.Controllers
{
    ...
    public class ScopeControllerTests
    {
        ...
        public async Task ScopeController_InScope_GET_When_User_is_Admin_then_Redirect_to_Home_Admin()
        {
            ...
            // Assert
            Assert.NotNull(result, "RedirectToActionResult should not be null");
            Assert.AreEqual("AdminHomePage", result.ActionName, "Expected the Action to be 'Home'");
            Assert.AreEqual("AdminHomepage", result.ControllerName, "Expected the Controller to be 'Admin'");
        }
        ...
        public async Task ScopeController_ConfirmInScope_POST_When_Scope_is_Changed_to_In_For_Current_Year_Then_Email_Users()
        {
            ...
            UiTestHelper.MockBackgroundJobsApi.Verify(
                x => x.AddEmailToQueue(It.Is<NotifyEmail>(inst => inst.TemplateId.Contains(EmailTemplates.ScopeChangeInEmail))),
                Times.Exactly(2),
            ...
            UiTestHelper.MockBackgroundJobsApi.Verify(
                x => x.AddEmailToQueue(It.Is<NotifyEmail>(inst => inst.EmailAddress.Contains(user3.EmailAddress))),
                Times.Never,
        ...
        public async Task ScopeController_OutOfScope_GET_When_User_is_Admin_then_Redirect_to_AdminAsync()
        {
            ...
            // Assert
            Assert.NotNull(result, "RedirectToActionResult should not be null");
            Assert.AreEqual(result.ActionName, "AdminHomePage", "Expected the Action to be 'Home'");
            Assert.AreEqual(result.ControllerName, "AdminHomepage", "Expected the Controller to be 'Admin'");
        }
        ...
        public void ScopeController_ConfirmOutOfScopeDetails_GET_When_CurrentUser_Is_Admin_Then_Redirect_To_Home_Admin()
        {
            ...
            // Assert
            Assert.NotNull(result, "RedirectToActionResult should not be null");
            Assert.AreEqual("AdminHomePage", result.ActionName, "Expected the Action to be 'ManageOrganisations'");
            Assert.AreEqual("AdminHomepage", result.ControllerName, "Expected the Controller to be 'Organisation'");
        }
        ...
        public async Task ScopeController_ConfirmOutOfScopeAnswers_POST_When_Scope_is_Changed_to_Out_For_Current_Year_Then_Email_Users()
        {
            ...
            UiTestHelper.MockBackgroundJobsApi.Verify(
                ...
                "Expected the current user's email address to be in the email send queue");
            UiTestHelper.MockBackgroundJobsApi.Verify(
                x => x.AddEmailToQueue(It.Is<NotifyEmail>(inst => inst.TemplateId.Contains(EmailTemplates.ScopeChangeOutEmail))),
                Times.Exactly(2),
